...
1. Open the ApacheJMeter.jar in "apache-jmeter-5.6.3\bin" where you installed ApacheJemeter 5.6.3
2. Click File in the top right of the program then Open, now open either of the .jmx files
3. Change the File Path under File Upload in the HTTP requests to be where the files are placed on your computer
4. Change the HTTP request's IP address to be your server's (the IP of the machine you are running it on if you use the upload_server.py)
5. Set the amount of thread users and Ramp up time in Thread Group To run the test plan click on with the Green arrow on the bar near the top of the window that should say start when you hover over it
